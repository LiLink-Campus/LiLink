  ...
  async schoolsGender(
    ...
    >(Prisma.sql`
      SELECT
        ...
        COUNT(*)::int AS "count",
        COUNT(base."submittedAt")::int AS "submitted"
      FROM (
        SELECT
        ...
        WHERE u."deactivatedAt" IS NULL
          ${
            query.cycleId
              ? Prisma.sql`AND u."status" = 'ACTIVE' AND EXISTS (
            SELECT 1 FROM "CycleParticipation" cp
            WHERE cp."userId" = u."id" AND cp."cycleId" = ${query.cycleId}
              AND cp."status" = 'OPTED_IN' AND cp."intent" IS NOT NULL
          )`
              : Prisma.empty
          }
