...
function termsText(key: "title" | "intro" | "free") {
  return (
    <LocalizedText
      zh={TERMS_COPY["zh-CN"][key]}
      en={TERMS_COPY["en-US"][key]}
    />
  );
}
  ...
  return (
    <main className="page-shell prose-shell">
      <section className="content-panel">
        ...
        <h1>{termsText("title")}</h1>
        <p>{termsText("intro")}</p>
        <p>{termsText("free")}</p>
        <ul>
          {TERMS_COPY["zh-CN"].items.map((item, index) => (
            <li key={item}>
              <LocalizedText zh={item} en={TERMS_COPY["en-US"].items[index]} />
            </li>
          ))}
        </ul>
      </section>
