...
function firstStepForScope(scope: MeetupProposalScope) {
  return scope === "LOCATION_ONLY" ? 2 : 1;
}
...
function MeetupSessionView({
  ...
  const proposalDefaultScope = defaultScopeForSession(session);
      ...
      <div className="app-page-shell app-page-shell-narrow v2-page-shell">
        ...
        {actionState === "needsPropose" ? (
          <section className="app-card">
            ...
            <MeetupProposalForm
              key={`${session.id}-${proposalDefaultScope}`}
              defaultScope={proposalDefaultScope}
...
function MeetupProposalForm({
  ...
  const [step, setStep] = useState<1 | 2 | 3>(() =>
    firstStepForScope(defaultScope),
  );
