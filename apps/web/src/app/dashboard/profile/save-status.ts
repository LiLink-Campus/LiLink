export type SaveState = "idle" | "pending" | "saving" | "retrying" | "draft-saved" | "submitted" | "error";

export function profileSavePresentation(state: SaveState, submitted: boolean, draft: boolean, dirty: boolean, incomplete = false) {
  if (state === "error") return { label: "保存失败", detail: "修改尚未保存，请重试后再离开。", tone: "error" };
  if (state === "retrying") return { label: "正在重试保存…", detail: "网络暂时不稳定，修改尚未确认保存，请暂勿离开。", tone: "pending" };
  ...
  if (incomplete && submitted) return { label: "资料待补全", detail: "当前资料有必填项待补全，请检查下方提示。", tone: "draft" };
