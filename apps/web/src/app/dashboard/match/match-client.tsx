    ...
    hero = (
      <MatchStateHero
        ...
        actions={
          [
            {
              label: saving === "contact" ? "处理中..." : "发起见面邀请",
              onClick: () => void handleDirectInvite(),
              variant: "primary",
              disabled: saving === "contact",
              loading: saving === "contact",
              className: "v2-match-action-primary",
            },
            introduced
              ? {
                  label: "已引荐",
                  variant: "secondary",
                  disabled: true,
                  className: "v2-match-action-disabled",
                }
              : {
                  label: saving === "contact" ? "处理中..." : "交换联系方式",
                  onClick: () => void requestContact(latestMatch.id),
                  variant: "secondary",
                  disabled: saving === "contact",
                  loading: saving === "contact",
                  className: "v2-match-action-secondary",
                },
          ]
        }
      >
        {introduced && reportLabel ? (
          <div className="v2-match-report-chip-row">
            <span className="domain-chip">{reportLabel}</span>
          </div>
        ) : null}

        {counterpart?.introLine ? (
          <p className="v2-match-hero-body v2-match-hero-intro-line">
            <strong>对方介绍：</strong>
            {counterpart.introLine}
          </p>
        ) : null}
        ...
        {showStartMeetupRibbon && latestMatch ? (
          <div className="v2-match-hero-section">
            <Link
              ...
              className="v2-match-start-card"
              aria-label="安排第一次见面"
            >
              <div className="v2-match-start-card-head">
                <strong>下一步 · 安排第一次见面</strong>
                <span className="v2-match-start-card-pill">
                  <span />
                  可发起
                </span>
              </div>
              <p className="v2-match-start-card-body">
                引荐已完成，现在可以给 {counterpart?.displayName ?? "对方"} 提议 2–3 个时间段和地点。
              </p>
              <div className="v2-match-start-card-link">
                进入安排页面 &gt;
              </div>
            </Link>
          </div>
    ...
    <div className="app-page-shell app-page-shell-narrow v2-page-shell">
      <header className="v2-greeting">
        <div className="v2-greeting-main">
          <h1 className="v2-match-page-title">你本周的匹配对象</h1>
          <p className="v2-greeting-sub v2-match-page-subtitle">
            {introduced
