...
export function BirthDatePicker({value, minYear, maxYear, onChange}: {value: string; minYear: number; maxYear: number; onChange: (value: string) => void}) {
  ...
  return <>
    <button type="button" className={styles.trigger} aria-label="选择出生日期" onClick={open}>{value ? value.replaceAll("-", "/") : "请选择出生日期"}<span aria-hidden="true"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M7 3v4M17 3v4M3 11h18"/></svg></span></button>
