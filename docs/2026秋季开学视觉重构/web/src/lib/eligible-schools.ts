import { isVisualPreview } from "./visual-preview/mode";
...
export async function fetchEligibleSchools(): Promise<EligibleSchoolsPayload> {
  const response = await fetch(isVisualPreview() ? "/api/visual-preview?path=/public/schools" : `${getClientApiBaseUrl()}/public/schools`, {
