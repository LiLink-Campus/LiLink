...
import { VISUAL_PREVIEW, isVisualPreview } from "@/lib/visual-preview/mode";
...
export default function ForgotPasswordPageClient() {
  ...
  async function requestCode() {
    if (isVisualPreview()) { setCodeSent(true); return; }
  ...
  function continueToPassword(event: FormEvent<HTMLFormElement>) {
    ...
    if (!VISUAL_PREVIEW && (!codeSent || !/^\d{6}$/.test(code))) { setError("请先获取并填写 6 位验证码。"); return; }
    setError(null);
    setStep(2);
  }
  ...
  async function resetPassword(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (isVisualPreview()) { window.location.href = "/login"; return; }
    setPending(true);
    setError(null);
  ...
  return (
    ...
    >
      ...
      <Card className={`${authStyles.panel} ${step === 1 ? loginStyles.card : flowStyles.panel} ${styles.panel} animate-in`} layout="plain">
        <h1>重置密码</h1>
        {step === 1 ? (
          <p className={styles.subtitle}>输入注册邮箱和验证码，继续设置新密码。</p>
        ) : (
          <p className={`${styles.subtitle} ${styles.stepTwoSubtitle}`}>设置新密码，提交时将核验邮箱验证码。</p>
        )}

        {step === 1 ? (
          <form noValidate={VISUAL_PREVIEW} className={authStyles.stack} onSubmit={continueToPassword}>
            <Field label="注册邮箱">
              <Input
                required
                type="email"
          ...
          <form noValidate={VISUAL_PREVIEW} className={authStyles.stack} onSubmit={resetPassword}>
            <div className={flowStyles.emailSummary}>
              <span>已发送到</span>
              <strong>{email || (VISUAL_PREVIEW ? "student@example.com" : "")}</strong>
            </div>
            <Field label="新密码">
              <Input
                required
