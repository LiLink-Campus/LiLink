...
function MeetupProposalForm({
  ...
  const [selectedTimes, setSelectedTimes] = useState<TimeSlot[]>(() => VISUAL_PREVIEW ? [18, 19].map(hour => ({
    key: `preview-${hour}`, startsAt: `${new Date(Date.now() + 86400000).toISOString().slice(0, 10)}T${hour}:00`,
    endsAt: `${new Date(Date.now() + 86400000).toISOString().slice(0, 10)}T${hour + 1}:00`,
  })) : []);

  // Location state
  const [selectedLocations, setSelectedLocations] = useState<LocationSlot[]>(() => VISUAL_PREVIEW ? ["校园湖畔咖啡馆", "图书馆前广场"].map((placeName, index) => ({ key: `preview-place-${index}`, kind: "custom", placeName })) : []);
