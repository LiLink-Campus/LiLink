...
import { isVisualPreview, VISUAL_PREVIEW } from "@/lib/visual-preview/mode";
...
export default function RegisterSchoolClient() {
  ...
  async function requestCode(event: { preventDefault: () => void }) {
    ...
    if (isVisualPreview()) { setCodeSent(true); setError(null); setResendCooldown(RESEND_COOLDOWN_SECONDS); return; }
    setError(null);
  ...
  function continueRegistration(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!isVisualPreview() && (!codeSent || !/^\d{6}$/.test(code))) {
  ...
  async function register(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (isVisualPreview()) { window.location.href = "/dashboard"; return; }
    setError(null);
  ...
  return (
    ...
    >
      {step === 1 ? (
        <form noValidate={VISUAL_PREVIEW} className={authStyles.stack} onSubmit={continueRegistration}>
          <Field
            label="学校邮箱"
          >
            <Input
          ...
          {schoolsPayload ? (
            <SchoolEmailSupport email={email} payload={schoolsPayload} />
          ) : null}
          {schoolsError ? (
            <SchoolListRetry
        ...
        <form noValidate={VISUAL_PREVIEW} className={authStyles.stack} onSubmit={register}>
          <Field
            label="邀请码（可选）"
