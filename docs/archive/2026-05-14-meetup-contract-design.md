...
> 来源提交：`a6b1bf4abd4e230be348e7f9316ba7c3b5f54190`；原文件：`docs/破冰/meetup-contract-design.zh-CN.md`。
...
- 如果用户未触碰引荐，且对方发出见面提议后一天内未采取任何操作，则每个见面会话最多发送一次邮件提醒；发送链路待定，不在本次实现范围内落地。

### LIMITED / visibility 当前语义

- 现有 dashboard 和匹配历史 DTO 继续保留 `visibility` / `LIMITED` 字段，用于表达举报、屏蔽等导致的可识别信息隐藏状态。
- Meetup V1 暂不设计完整的 visibility / policy 系统。`LIMITED` 不是阻止参与者访问既有 `MeetupSession` 的硬门槛；既有会话的 `GET session`、meetup summary、`POST /seen` 和可用操作判断仍以当前用户是否为会话参与者、匹配是否已引荐、会话状态和操作前置条件为准。
- `visibility` 字段保留给后续更完整的 visibility / policy 设计。未来若要让 `LIMITED` 触发自动取消、隐藏、阻止新建或限制 mutation，必须在本文档中补齐明确规则、迁移路径和审计事件，不能把字段名本身当作隐式授权逻辑。
...
所有路由都要求 `JwtAuthGuard`。用户只能访问自己是参与者且底层匹配已引荐的会话；`LIMITED` 当前不是既有 meetup 的硬访问门槛。
...
- `meetupSummary` 是实时权威 API 数据，不是 dashboard snapshot 字段。
...
Precondition:
  ...
  match visibility is VISIBLE for both users when starting a new session; this is not reused as an access gate for an existing session
...
状态：发送链路待定，不在本次实现范围内落地。本节保留业务需求、触发条件、去重约束和审计要求，作为后续实现 reminder job / email queue / flush 链路时的依据。
...
后续实现首选：使用 `OutboundEmail.dedupeKey`；如果仓库已经有后台邮件 flush，再增加 scheduled job/service。

提醒按会话去重，不按用户或提议消息去重。同一会话内后续的反提议或锁定后修改提议，在已有该 `sessionId` 的提醒入队后，不得再次入队提醒。

邮件不应暴露超过已引荐匹配本身允许范围的敏感对方信息。不要为已取消、已过期或已归档的匹配排队提醒。

## 安全与访问控制

- `GET session` 只返回当前用户是 `MeetupParticipant` 的会话。
- Dashboard task、meetup summary、`/seen`、提醒入队只依赖见面会话参与者身份、匹配引荐状态和会话状态。
- V1 不把 `Match.visibility = LIMITED` 当作既有 meetup 的统一拒绝访问条件；该字段当前只承载可识别信息展示限制，完整 visibility / policy 留给后续设计。
...
- `meetup.expired`
- `meetup.archived`
- `meetup.visibility_canceled`（后续 visibility / policy 设计启用时使用）
- `meetup.seen`
- `meetup.reminder_queued`（发送链路实现后启用）
