...
> 来源提交：`3097f6e8116f241f73204368cb07963b84798363`；原文件：`docs/merchant-promotion-contract-design.zh-CN.md`。
...
### §A 优惠规则建模 + §B 核销求值 — 已定稿并实现（2026-05-23）

两个「待设计模块」已定稿为**阶梯券（tier ladder）**模型并落地，详见 `merchant-coupon-rule-and-redemption.zh-CN.md`。要点：

- §A：`rule = { version:1, tiers:[{ minSpend, benefit }] }`，benefit ∈ 满减/折扣/满赠；`benefitType` 与档类型强一致；`CUSTOM` 无 rule。`@lilink/shared` 的 `validateCouponRule(rule, benefitType)` / `evaluateCoupon` / `requiresOrderAmount` / `renderBenefitText` 由占位 stub 换为真实现。
- §B：`RedeemCouponDto.orderAmount?` 启用；核销「先求值后消费」，结果由 3 态扩为 5 态（新增 `NEED_AMOUNT` / `BELOW_THRESHOLD`，均不消费券、不泄露存在性）；`Redemption.orderAmount/actualDiscountAmount` 落库，赠品身份记审计。
- **零 schema 迁移**（数据位 M0 已预留）。后台券模板编辑器加阶梯编辑（含折扣），核销页加消费额输入与 5 态渲染。
- 验证：shared 77 测试、api 454 测试、web typecheck/lint、全量 lint 均通过。
