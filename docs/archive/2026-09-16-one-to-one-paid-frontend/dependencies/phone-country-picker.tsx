...
const countries = getCountries().map((country) => ({
  country,
  name: shortNames[country] ?? chineseNames.of(country) ?? country,
  searchNames: [chineseNames.of(country), englishNames.of(country), shortNames[country], country].join(" ").toLowerCase(),
  code: getCountryCallingCode(country),
}));
const common = commonCountries.flatMap((country) => countries.filter((item) => item.country === country));
const remaining = countries.filter((item) => !commonCountries.includes(item.country)).sort((a, b) => a.name.localeCompare(b.name, "zh-CN"));
