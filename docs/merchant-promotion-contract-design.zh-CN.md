...
```ts
// @lilink/shared/coupon.ts（待补全）
export type CouponRule = /* 待设计：扁平对象 | DSL | ... */ unknown;
export function evaluateCoupon(rule: CouponRule, ctx: { orderAmount?: number; now: Date }):
  { ok: boolean; reason?: string; computedDiscount?: number };   // 核销求值
export function renderBenefitText(rule: CouponRule): string;       // 展示成人话（券页 / 核销页）
export function requiresOrderAmount(rule: CouponRule): boolean;    // 是否需要商家输入消费额（驱动 §B）
export function validateCouponRule(rule: unknown): CouponRule;     // 创建券模板时校验
```
