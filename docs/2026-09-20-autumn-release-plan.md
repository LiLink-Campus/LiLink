...
当前撮合在 API 内同步调用 Blossom，未发现独立 worker；揭晓提交后再批量同步快照。这两处是重点测量对象。只有并发实测证明 API 被阻塞后再评估 worker/队列或资源升级，不能把历史求解器微基准当成生产容量结论。
...
压测执行前产出可复跑脚本、合成数据生成与清理脚本、目标库硬校验、资源上限/费用边界。执行后交付提交 SHA、配置、数据规模、实际请求率、p50/p95/p99、错误分类、业务断言、资源曲线、瓶颈与清理结果。本次仅制定计划，没有生成负载或声称通过这些指标。
