# 优惠券阶梯规则与核销求值（合同 §A / §B 定稿）

本文档为 `merchant-promotion-contract-design.zh-CN.md` 末尾两个「待设计模块」的定稿实现说明：
...
`build:shared` / `test:shared`(77)、`build:api` / api jest(454)、`typecheck:web` / `lint:web` / `lint:css` / `lint` 均通过。
